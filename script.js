...
function animateText() {
    if (isTyping) {
        if (index < text.length) {
            ...
            isTyping = false; // Alterna para o modo de pagamento
        }
    } else {
        if (index > 1) {
            ...
            ('light') ? 'black' : '#fff') ? '#C94C16' : (document.documentElement.classList.contains
            ('light') ? 'black' : '#fff');
...
// Inica a animação do título ao carregar a página
...
homeSection.style.transition = 'opacity 1 ease, transform 1s ease';
...
contactForm.addEventListener('submit', (e) => {
    ...
    .then(response => {
        if (response.ok) {
            ...
            alert('Erro ao enviar fomulário. Tente novamente.');
        }
    })
